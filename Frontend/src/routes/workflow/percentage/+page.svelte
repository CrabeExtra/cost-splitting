<script>
    ...
    let title = 'Create percentage Expense';

    let subtitle =
        'Create an expense where each person pays a percentage of the total amount based on external factors.';
...
>
    <div class="people-section">
        ...
        {#each selectedPeople as person, index}
            ...
            <input
                list={person.localId + '-percentage'}
                placeholder="Search or enter percentage as an integer"
                type="number"
                value={person.percentage}
                oninput={(e) => {
                    
                    if(Number(e.target.value) !== 0) {
                        person.percentage = e.target.value
                    } else {
                        e.target.value = '';
                    }
